...
Fake metric values can also be updated at runtime via [`POST /admin/config`](api.md#adminconfig) with a `fake-metrics` field — the body is a partial update, so only the specified metrics are changed.
