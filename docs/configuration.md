...
- `engine`: the inference engine backend to simulate, optional, default is `vllm` (the only supported value). Determines which flags, environment variables, metric names, and KV-event format the simulator uses; see [Engine backends](engine-backends.md). If you omit `--engine` on the command line, a non-empty `SIM_ENGINE` environment variable can supply it; see [Configuration precedence](#configuration-precedence) and [Environment variables](#environment-variables).
...
- `latency-calculator`: specifies the latency calculator for prefill time. Supported values are `constant` and `per-token`; see [Latency Simulation](latency-simulation.md) for details on each. Leaving this field unset (or setting it to `""`) activates a legacy precedence-based mode — retained for backward compatibility, but using `constant` or `per-token` explicitly is recommended. This is a top-level flag; it is not nested under `latencies:`.

Command-line flag names for the remaining latency parameters are as listed below. In a YAML config file, these settings may be nested under a top-level `latencies:` key using the same key names; the old flat top-level keys are still accepted for backward compatibility.
...
- `zmq-topic`: ZMQ topic to publish KV-cache events under. Empty (the default) uses the llm-d convention `kv@<pod-ip>:<serving-port>@<model>`. When set, the value replaces that topic verbatim, matching vLLM's static `KVEventsConfig.topic`, for subscribers that expect a fixed name. Note that a static topic is shared by every rank in a data-parallel deployment, unlike the default.
...
- `SIM_ENGINE`: when non-empty and **`--engine` is not passed on the command line**, selects the engine backend and overrides the `engine` value from the YAML file (if any) and the default. If you pass `--engine`, it always wins. See [Engine backends](engine-backends.md).
