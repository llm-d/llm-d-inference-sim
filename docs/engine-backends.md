...
only registered backend.

## Selecting a backend

The `engine` setting picks the backend: see [Configuration](configuration.md#general) for the flag and
...
backend registers its own flags and validates its own fields as part of that parse. `engine.Select` then
...
| --- | --- | --- |
| `Name` | The backend's identifier, also reported as `owned_by` in `/v1/models` | [vllm.go](../pkg/engine/vllm/vllm.go) |
...
A backend inherits these rather than reimplementing them. They are the parts most likely to be mistaken
for engine-specific:
...
3. Document the backend's own flags, environment variables, and metric names in
...
4. Add a test suite in `pkg/engine/<name>/` for the parts with an external contract: the KV-event encoder
   ...
   way `main` does, so setting `SIM_ENGINE` runs them against a different backend.

## Import-cycle constraint

`pkg/engine` imports `pkg/communication` for its transport hooks, so no package it imports can import it
back. Each consumer declares the narrow interface it needs instead, and the engine satisfies all of them
structurally:

| Interface | Needs |
| --- | --- |
| `engine.Engine` | Everything, as the registry's value type |
| `common.Engine` | `Name`, `ApplyDefaults`, `BindFlags`, `ApplyEnv`, `ValidateConfig` |
| `simulator.Engine` | `ValidateConfig`, `NewMetricsAdapter`, `NewKVEventEncoder` |
| `communication.Transport` | `BindHTTP`, `BindGRPC` |

A new hook goes on `engine.Engine` plus whichever consumer interface calls it, and its types belong to the
package that consumes them: `EventEncoder` lives in `pkg/kvcache` because `pkg/kvcache` is what calls it,
and `pkg/metrics` cannot supply one because `pkg/kvcache` already imports `pkg/metrics`.
