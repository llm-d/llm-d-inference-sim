...
- `/v1/responses`
    - **request**
        - stream
        - model
        - input (string, or array of input message objects)
            - type (`message`)
...
- `length` — generation stopped because the `max_tokens` / `max_completion_tokens` / `max_output_tokens` limit was reached.
