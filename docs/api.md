...
- **`POST /admin/config`** applies a partial JSON update and returns the new configuration. The request body must be a JSON object whose keys are a subset of the admin-configurable fields:
  ...
  - `fake-metrics` — partial update of [fake metric](configuration.md#fake-metrics) values. The value is itself a JSON object containing only the metrics to change; any metrics not specified are left unchanged. When the simulator was started without `--fake-metrics`, the first non-null `fake-metrics` update switches that process into fake mode (real metrics stop updating). When fake metrics were already configured at startup, only the specified fields change.
  ...
  - `latency-calculator` — string; same accepted values as the `--latency-calculator` flag. This is a flat, top-level field; unlike the latency fields below, it is not part of the nested `latencies` object, is always returned at the top level by `GET`, and is rejected with `400 Bad Request` if sent nested inside `latencies`.
  - Latency-related fields: `time-to-first-token`, `time-to-first-token-std-dev`, `inter-token-latency`, `inter-token-latency-std-dev`, `kv-cache-transfer-latency`, `kv-cache-transfer-latency-std-dev`, `prefill-overhead`, `prefill-time-per-token`, `prefill-time-std-dev`, `kv-cache-transfer-time-per-token`, `kv-cache-transfer-time-std-dev`, `time-factor-under-load` (float). Duration fields accept a Go duration string (e.g. `"250ms"`, `"1s"`). The same validation rules apply as at startup (no negative values; std-dev ≤ 30 % of base; `time-factor-under-load` ≥ 1.0). Updates take effect on subsequent requests.
