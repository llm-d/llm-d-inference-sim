...
# Data Types
##############################
# Event tags matching Go constants
...
def unmarshal_kv_event(raw_event: bytes) -> tuple[EventType, str]:
...
def listener():
    ...
    if not port:
        logger.info("ZMQ_PORT environment variable is not defined")
        return
    ...
    logger.info(f"Listener is ready and waiting for messages on tcp://*:{port}")
        ...
        while True:
            ...
            if socks.get(socket) == zmq.POLLIN:
                ...
                for raw_event in event_batch.events:
                    ...
                    # Log event
                    logger.info(f"Event: {event.to_tagged_union()}")
