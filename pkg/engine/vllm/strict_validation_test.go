...
var _ = Describe("strict vLLM request validation", func() {
	...
	DescribeTable("matches vLLM sampling errors",
		func(tc invalidCase) {
			...
			err := validateSamplingTestBody([]byte(tc.body), path)
	...
	It("accepts vLLM boundary values", func() {
		...
		Expect(validateSamplingTestBody(body, chatCompletionsPath)).To(BeNil())
	})
	...
	It("defers malformed field types to normal request decoding", func() {
		Expect(validateSamplingTestBody([]byte(`{"temperature":"cold"}`), chatCompletionsPath)).To(BeNil())
	})

	It("serializes stable validation errors byte-for-byte like vLLM", func() {
		err := validateSamplingTestBody([]byte(`{"top_p":0}`), chatCompletionsPath)
...
func BenchmarkStrictCompletionValidation(b *testing.B) {
	...
	b.Run("valid", func(b *testing.B) {
		b.ReportAllocs()
		for range b.N {
			if err := validateSamplingTestBody(valid, chatCompletionsPath); err != nil {
				b.Fatal(err)
			}
		}
	})
	...
	b.Run("invalid", func(b *testing.B) {
		b.ReportAllocs()
		for range b.N {
			if err := validateSamplingTestBody(invalid, chatCompletionsPath); err == nil {
...
func validateSamplingTestBody(body []byte, path string) *api.Error {
