...
func (s *VllmSimulator) parseCommandParams() error {
	...
	f.StringSliceVar(&s.servedModelNames, "served-model-name", nil, "Model name exposed by the API (comma or space-separated)")
	var lorasStr string
	...
	if len(s.servedModelNames) == 0 {
		if s.model == "" {
			return errors.New("either --model or --served-model-name must be provided")
		}
...
func (s *VllmSimulator) sendResponse(isChatCompletion bool, ctx *fasthttp.RequestCtx, respTokens []string, toolCalls []toolCall,
	...
	// Decide what model name should appear in the JSON payload.
	displayedModelName := s.getDisplayedModelName(model)
	resp := s.createCompletionResponse(isChatCompletion, respTokens, toolCalls, &finishReason, usageData, displayedModelName)
